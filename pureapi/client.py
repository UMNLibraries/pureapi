...
def get(endpoint, params={}, headers=headers, retryer=retryer):
    ...
    with requests.Session() as s:
        ...
        try:
            ...
            raise PureAPIClientHTTPError(
                f'GET request for endpoint {endpoint} with params {params} returned HTTP status {http_exc.response.status_code}',
                request=http_exc.request,
                response=http_exc.response
            ) from http_exc
        except RequestException as req_exc:
            raise PureAPIClientRequestException(
                f'Failed GET request for endpoint {endpoint} with params {params}',
                request=req_exc.request,
                response=req_exc.response
            ) from req_exc
        except Exception as e:
            raise PureAPIClientException(
                f'Unexpected exception for GET request for endpoint {endpoint} with params {params}'
            ) from e
...
def filter(endpoint, payload={}, headers=headers, retryer=retryer):
    with requests.Session() as s:
        ...
        try:
            ...
            raise PureAPIClientHTTPError(
                f'GET request for endpoint {endpoint} with payload {payload} returned HTTP status {http_exc.response.status_code}',
                request=http_exc.request,
                response=http_exc.response
            ) from http_exc
        except requests.exceptions.RequestException as req_exc:
            raise PureAPIClientRequestException(
                f'Failed POST request for endpoint {endpoint} with payload {payload}',
                request=req_exc.request,
                response=req_exc.response
            ) from req_exc
        except Exception as e:
            raise PureAPIClientException(
                f'Unexpected exception for POST request for endpoint {endpoint} with payload {payload}'
            ) from e
